...
Decyzja o~stanie klucza opiera się na~liniowej powierzchni przełączającej $S(k)$, łączącej uchyb regulacji napięcia oraz uchyb prądowy:
\begin{equation}
...
\end{equation}
gdzie $w_i > 0$ jest~bezwymiarową wagą członu prądowego. Dyskretne polecenie sterujące $s_{\mathrm{cmd}}(k)\in\{0,1\}$ wyznaczane jest~bezpośrednio przez~funkcję progową (komparator):
\begin{equation}
    s_{\mathrm{cmd}}(k) =
    \begin{cases}
        1, & \text{dla } S(k) \ge 0, \\
...
Warto odnotować istotny szczegół formalno-matematyczny: w~oryginalnym druku artykułu~\cite{tatari2024mfbb} w~równaniu definiującym powierzchnię przełączającą omyłkowo wydrukowano moduł uchybu prądu $\lvert i_{\mathrm{des}} - i_L\rvert$. Dla~prawidłowej realizacji sterowania przekaźnikowego niezbędne jest~jednak zachowanie informacji o~kierunku błędu, stąd w~rzeczywistej implementacji stosuje się uchyb ze~znakiem~\eqref{eq:mfbb-errors} i~bezpośrednie porównanie z~zerem~\eqref{eq:mfbb-comparator}.
